...
| `lyrics-index` | lyrics list | published `files/translation/lyrics/index.json`; IDs, revision, timestamps, localized title | music ID / NEXT public contract v1 | published subset | published subset | partial; absent songs not claimed |
| `lyrics-detail` | lyrics detail | published `files/translation/lyrics/music_{musicId}.json`; Japanese, zh-CN, en-US, segments, performers, attribution | music ID + revision + line ID / NEXT public contract v1 | per-line + JP fallback | per-line + JP fallback | partial; absent lines not claimed |
