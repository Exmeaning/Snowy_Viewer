...
| `lyrics-index` | lyrics list | published `files/translation/lyrics/index.json`; IDs, revision, timestamps, localized title, v2 availability state, Full/Game versions, no-lyrics reason | music ID / NEXT public contracts v1-v2 | published subset | published subset | partial; all catalog states may be indexed, while unresolved/no-lyrics states have no detail |
| `lyrics-detail` | lyrics detail | published `files/translation/lyrics/music_{musicId}.json`; complete or Game-only state, translations, ruby, segmented performers, whole-line trailing performers, public attribution, optional ordered Game projection | music ID + revision + line ID / NEXT public contracts v1-v2 | per-line + JP fallback | per-line + JP fallback | partial; details exist only for complete and Game-only states |
| `lyrics-performers` | lyrics detail | segment performer IDs, whole-line trailing performer IDs, CHAR_COLORS, and accessible names | character ID / performerIds or trailingPerformerIds | color/gradient text when segmented, line-end avatars for whole-line attribution | color/gradient text when segmented, line-end avatars for whole-line attribution | covered |
