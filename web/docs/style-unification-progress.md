...
| **Milestone 2** | 全局布局、顶栏、侧边栏、导航、环境背景光晕 | `[ ] 未开始` | - |
| **Milestone 3** | 核心数据库页面（卡牌、歌曲、活动、扭蛋、角色） | `[ ] 未开始` | - |
...
- [ ] **全局背景升级**：在 `web/src/components/MainLayout.tsx` 注入动态彩色漂浮 Blob 背景，并支持角色主题色关联。
- [ ] **侧边栏导航重构**：将 Sidebar 改为重型磨砂玻璃，高光选中态。
- [ ] **面包屑与顶栏重构**：将顶部面包屑栏和动作按钮改用轻量悬浮毛玻璃。
- [ ] **验证步骤**：
    - [ ] 运行本地 lint。
    - [ ] 验证跨屏幕响应式状态。

### Milestone 3: 核心数据库页面重构
- [ ] **卡牌列表与详情**：重构 `/cards` 及 `/cards/[id]` 详情面板、星级与属性容器。
- [ ] **歌曲列表与详情**：重构 `/music` 及 `/music/[id]` 别名栏、谱面预览和声优选项。
- [ ] **活动列表与详情**：重构 `/events` 及 `/events/[id]` 排行榜、加成角色及单位卡片。
- [ ] **扭蛋与角色**：重构 `/gacha` 与 `/character` 模块。
