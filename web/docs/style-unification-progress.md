...
| **Milestone 4** | 复杂工具页面（队伍推荐、对比、贴纸制作、联机游戏） | `[ ] 未开始` | - |
| **Milestone 5** | 剧情、漫画阅读器与详情模块 | `[ ] 未开始` | - |
...
- [x] **全局 CSS 定义**：在 `web/src/app/globals.css` 中设计并写入首批 `ios-glass-*` 实用类及过渡动画。
...
- [ ] **队伍推荐 & 对比器**：重构 `/deck-recommend` 筛选器面板、计算结果与卡组拖拽框。
- [ ] **贴纸制作**：重构 `/sticker-maker` 画布控制层及图层选择栏。
- [ ] **联机游戏**：重构 `/guess-who` 和 `/guess-jacket` 猜角色/猜曲绘匹配大厅与答题板。

### Milestone 5: 剧情、漫画阅读器与详情
- [ ] **故事列表与详情**：重构剧情树、分组标签页及锁头遮罩。
- [ ] **故事阅读器**：重构小说对话框、背景切换器、自动播放进度栏。
- [ ] **漫画阅读器**：重构 `/comic` / `/manga` 翻页悬浮球及双语对照浮窗。
