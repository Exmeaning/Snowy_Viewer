...
> 更新时间：2026-05-23  
> 适用范围：`web/` Next.js standalone 前端应用（页面交互以 CSR 为主）
...
- [x] **实时排行榜**：重构 `/realtime-ranking` 容器与控制面板，并新增玩家实时追踪（Tracking Mode）功能。
...
- [ ] **standalone 构建校验**：运行 `npm run build:next --prefix web` 确保服务端构建与客户端页面均可交付。
