...
- `src/lib/structured-data.ts` 承接 Root JSON-LD 与结构化数据 helper，并在硬编码中文扫描中作为 SEO/Project SEKAI 别名边界单独 allowlist；Root 已新增 registry-backed 站点导航 `ItemList`，详情页自动输出 `BreadcrumbList` JSON-LD，本轮核心入口页也通过 `withPageBreadcrumb()` 输出 registry-aware `BreadcrumbList`。
- `scripts/generate-sitemaps.mjs` 与 `scripts/generate-metadata-map.mjs` 本轮接入 guides 索引：detail sitemap 新增 1 条 `/guides/.../`，metadata map 新增 `guides: 1`。
