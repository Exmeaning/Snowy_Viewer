 ...
 * - When floating drawer is open (< 1280px): hidden to prevent overlapping the modal overlay.
 */
export default function FilterTabHandle({ isRailOpen }: FilterTabHandleProps) {
    ...
    // Do not render if page has no filters, or if floating modal drawer is currently open
    if (!hasFilters || (isOpen && !isDocked)) return null;
    ...
    const baseRailLeft = isRailOpen ? "md:left-[var(--hh-rail-w)]" : "md:left-0";

    const dockedOpenLeft = isRailOpen
        ? "xl:left-[var(--hh-dual-rail-w)]"
        : "xl:left-[var(--hh-filter-rail-w)]";

    const leftPositionClass = (isOpen && isDocked)
        ? dockedOpenLeft
        : `left-0 ${baseRailLeft}`;
    ...
    return (
        <AnimatePresence>
            <motion.button
                ...
                title={shortcutLabel}
                className={`fixed top-32 ${leftPositionClass} z-[58] flex flex-col items-center gap-1.5 py-3 px-1.5 rounded-r-[var(--hh-radius-md)] border border-l-0 bg-[var(--hh-surface-1)] border-[var(--hh-border)] shadow-sm hover:bg-[var(--hh-surface-sunken)] hover:border-[var(--hh-border-strong)] cursor-pointer transition-all duration-[var(--hh-dur-panel)] ease-[var(--hh-ease-out)] select-none group`}
            >
