...
export default function EventItem({ event, isSpoiler, basePath = "/events", unitType, bonusAttr, eventStoryIds }: EventItemProps) {
    ...
    return (
        ...
        >
            {/* Event Logo */}
            <div className="relative aspect-[16/9] bg-[var(--hh-surface-sunken)] overflow-hidden shrink-0">
                <Image
                    src={thumbnailUrl}
                    alt={event.name}
                    fill
            ...
            <div className="hh-card-footer p-2.5 sm:p-3 flex flex-col justify-between flex-1">
                ...
                {/* Event Name */}
                <h3 className="hh-title text-xs sm:text-sm font-bold text-[var(--hh-text-primary)] leading-snug min-h-[2.5rem] flex flex-col justify-start overflow-hidden mb-1">
                    <TranslatedText
                        original={event.name}
                        category="events"
                        field="name"
                ...
                {/* Date Range */}
                <div className="hh-body text-[10px] sm:text-xs text-[var(--hh-text-secondary)] space-y-0.5 mt-auto hidden sm:block">
