/**
 * Theme-reactive cursor system.
 *
 * Generates crisp SVG pointers whose accent geometry follows the active
 * character theme color, and exposes them as `--theme-cursor-*` custom
 * properties for `app/globals.css` to bind to real selectors.
 *
 * Two invariants make this work everywhere without a per-scheme variant:
 *
 * 1. EVERY GLYPH IS THREE LAYERS. A `#0D1117` outer contour keeps the pointer
 *    legible on white, a `#FFFFFF` inner core keeps it legible on black, and
 *    the theme color only ever appears as interior accent geometry. That is
 *    why light and dark mode can share one set of cursors — contrast comes
 *    from the glyph itself rather than from the surrounding page.
 *
 * 2. FALLBACKS ARE BAKED IN. Each value ends in a native keyword
 *    (`url(...) 2 2, default`), so a data-URI a browser refuses to decode
 *    degrades to the correct system cursor instead of to `auto`.
 *
 * `not-allowed` deliberately ignores the theme color and stays red: a disabled
 * affordance has to read as a refusal, not as decoration.
 */
...
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  <defs>
    <filter id="theme-cursor-shadow-def" x="-30%" y="-30%" width="160%" height="160%">
      <feDropShadow dx="0.8" dy="1.5" stdDeviation="1" flood-color="#000000" flood-opacity="0.38" />
    </filter>
  </defs>
  <g filter="url(#theme-cursor-shadow-def)">
...
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  <defs>
    <filter id="theme-cursor-shadow-ptr" x="-30%" y="-30%" width="160%" height="160%">
      <feDropShadow dx="0.8" dy="1.5" stdDeviation="1" flood-color="#000000" flood-opacity="0.38" />
    </filter>
  </defs>
  <g filter="url(#theme-cursor-shadow-ptr)">
...
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  <defs>
    <filter id="theme-cursor-shadow-txt" x="-30%" y="-30%" width="160%" height="160%">
      <feDropShadow dx="0.5" dy="1" stdDeviation="0.8" flood-color="#000000" flood-opacity="0.4" />
    </filter>
  </defs>
  <g filter="url(#theme-cursor-shadow-txt)">
...
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  <defs>
    <filter id="theme-cursor-shadow-grab" x="-30%" y="-30%" width="160%" height="160%">
      <feDropShadow dx="0.8" dy="1.5" stdDeviation="1" flood-color="#000000" flood-opacity="0.38" />
    </filter>
  </defs>
  <g filter="url(#theme-cursor-shadow-grab)">
...
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  <defs>
    <filter id="theme-cursor-shadow-grabbing" x="-30%" y="-30%" width="160%" height="160%">
      <feDropShadow dx="0.8" dy="1.5" stdDeviation="1" flood-color="#000000" flood-opacity="0.38" />
    </filter>
  </defs>
  <g filter="url(#theme-cursor-shadow-grabbing)">
...
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  <defs>
    <filter id="theme-cursor-shadow-na" x="-30%" y="-30%" width="160%" height="160%">
      <feDropShadow dx="0.8" dy="1.5" stdDeviation="1" flood-color="#000000" flood-opacity="0.38" />
    </filter>
  </defs>
  <g filter="url(#theme-cursor-shadow-na)">
...
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  <defs>
    <filter id="theme-cursor-shadow-help" x="-30%" y="-30%" width="160%" height="160%">
      <feDropShadow dx="0.8" dy="1.5" stdDeviation="1" flood-color="#000000" flood-opacity="0.38" />
    </filter>
  </defs>
  <g filter="url(#theme-cursor-shadow-help)">
...
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  <defs>
    <filter id="theme-cursor-shadow-zi" x="-30%" y="-30%" width="160%" height="160%">
      <feDropShadow dx="0.8" dy="1.5" stdDeviation="1" flood-color="#000000" flood-opacity="0.38" />
    </filter>
  </defs>
  <g filter="url(#theme-cursor-shadow-zi)">
...
<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  <defs>
    <filter id="theme-cursor-shadow-zo" x="-30%" y="-30%" width="160%" height="160%">
      <feDropShadow dx="0.8" dy="1.5" stdDeviation="1" flood-color="#000000" flood-opacity="0.38" />
    </filter>
  </defs>
  <g filter="url(#theme-cursor-shadow-zo)">
    <path d="M14.5 14.5L20.5 20.5" stroke="#0D1117" stroke-width="3.5" stroke-linecap="round"/>
    <path d="M14.5 14.5L20.5 20.5" stroke="${color}" stroke-width="1.8" stroke-linecap="round"/>
    <circle cx="9.5" cy="9.5" r="7" fill="#0D1117" stroke="#0D1117" stroke-width="1"/>
    <circle cx="9.5" cy="9.5" r="5.5" fill="#FFFFFF"/>
...
/**
 * Returns dictionary of all `--theme-cursor-*` CSS custom properties for a theme color
 */
export function getThemeCursorCssVariables(
    ...
    for (const key of Object.keys(CURSOR_CONFIGS) as ThemeCursorType[]) {
        variables[`--theme-cursor-${key}`] = getThemeCursorCssValue(key, themeColor);
    }
    return variables;
}
...
/**
 * Removes every `--theme-cursor-*` property, which is how the feature is turned
 * off rather than by rewriting stylesheets.
 *
 * `globals.css` declares each rule as `cursor: var(--theme-cursor-x, x)`, so
 * deleting the variable makes the declaration collapse to its native keyword —
 * the page reverts to system cursors with no rule churn and nothing left to
 * clean up if the user turns the setting back on.
 */
export function clearThemeCursors(target?: HTMLElement): void {
    if (typeof document === "undefined") {
        return;
    }
    const elem = target || document.documentElement;
    for (const key of Object.keys(CURSOR_CONFIGS) as ThemeCursorType[]) {
        elem.style.removeProperty(`--theme-cursor-${key}`);
    }
}
